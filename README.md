...
## Distribution NPM (bibliothèque)

Le composant **PersonForm** est exporté comme une **bibliothèque NPM**.

- Package : `test-cycle-tdd-person-form`
- Lien NPM : https://www.npmjs.com/package/test-cycle-tdd-person-form
...
### CI/CD (publication auto)

Le workflow GitHub Actions enchaîne :

1. **build-and-test** (build + tests unitaires + Cypress)
2. **publish-npm** (bypass si la version locale n’est pas strictement supérieure à celle sur NPM)
3. **deploy** (GitHub Pages), même si la publication NPM a été skippée

> Pour activer la publication : ajouter un secret GitHub `NPM_TOKEN` (token NPM avec droits de publication).